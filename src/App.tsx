...
export default function App() {
  ...
  const saveCollection = async () => {
    ...
    setStatus({ kind: 'idle' });
    try {
      const contents = dirty ? serializeCollection(loaded.collection) : loaded.originalRaw;

      await window.clara.saveCollection(loaded.filePath, contents);
      setLoaded({ ...loaded, originalRaw: contents });
      setDirtyPaths(new Set());
      setStatus({ kind: 'ok', message: `Saved ${loaded.filePath}` });
    } catch (error) {
      setStatus({
        kind: 'error',
        message: error instanceof Error ? error.message : String(error)
  ...
  const openRequestTab = (path: ItemPath) => {
    if (!loaded) {
      return;
    }
    const item = getItemByPath(loaded.collection.item, path);
  ...
  const closeRequestTab = (path: ItemPath) => {
    ...
    <div className={`app ${navigator.userAgent.includes('Mac') ? 'platform-mac' : ''}`}>
      <header className="titlebar">
        ...
        <div className="titlebar-actions">
          <button type="button" onClick={openCollection}>
            Open
          </button>
          <button
            type="button"
      ...
      {!loaded && (
        <main className="welcome">
          <div className="welcome-mark" aria-hidden>C</div>
          <h1>Open a Postman collection</h1>
          <p>Edit the JSON in your repository directly. No import or export cycle.</p>
          <button type="button" className="primary" onClick={openCollection}>
            Open collection
          </button>
        </main>
      )}
          ...
          <aside className="sidebar">
            ...
            <div className="collection-heading">
              ...
              <div>
                ...
                <span>{counts.folders} folders · {counts.requests} requests</span>
              </div>
            </div>
            <CollectionTree
